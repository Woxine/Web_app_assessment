...
# 关联表：用户-作物点赞（多对多）
user_likes_crops = db.Table('user_likes_crops',
...
# 关联表：用户-菜品点赞（多对多）
user_likes_meals = db.Table('user_likes_meals',
    db.Column('user_id', db.Integer, db.ForeignKey('user.id'), primary_key=True),
    db.Column('meal_id', db.Integer, db.ForeignKey('meal.id'), primary_key=True),
    db.Column('liked_at', db.DateTime, default=datetime.utcnow, nullable=False)
)
...
class User(UserMixin, db.Model):
    ...
    # 多对多关系：用户点赞的作物
    liked_crops = db.relationship('Crop', secondary=user_likes_crops,
                                  backref=db.backref('liked_by_users', lazy='dynamic'),
                                  lazy='dynamic')
    
    # 多对多关系：用户点赞的菜品
    liked_meals = db.relationship('Meal', secondary=user_likes_meals,
                                  backref=db.backref('liked_by_users', lazy='dynamic'),
                                  lazy='dynamic')
...
class Crop(db.Model):
    ...
    meals = db.relationship('Meal', secondary=meal_ingredients,
